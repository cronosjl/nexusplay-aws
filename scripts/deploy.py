...
def zip_service(service_name, source_file):
    ...
    # On force le nom interne à 'lambda_function.py' pour correspondre au Handler
...
def create_api_structure(lambda_arns):
    ...
    if not api:
        api = apigw_client.create_rest_api(name=api_name, endpointConfiguration={"types": ["REGIONAL"]})
        api_id = api["id"]
    else:
        api_id = api["id"]
    ...
    for path, service_key in [("games", "game-service"), ("users", "user-service")]:
        # On nettoie/crée la ressource
        resources = apigw_client.get_resources(restApiId=api_id)["items"]
        res = next((r for r in resources if r.get("pathPart") == path), None)
        res_id = res["id"] if res else apigw_client.create_resource(restApiId=api_id, parentId=root_id, pathPart=path)["id"]
        ...
        try:
            ...
            apigw_client.put_integration(restApiId=api_id, resourceId=res_id, httpMethod="GET", type="AWS_PROXY", integrationHttpMethod="POST", uri=uri)
        except: pass
