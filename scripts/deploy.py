...
def get_or_create_role():
    ...
    try:
        return iam.get_role(RoleName="NexusPlayLambdaRole")["Role"]["Arn"]
        ...
        iam.attach_role_policy(RoleName="NexusPlayLambdaRole",
...
def setup_sns():
    ...
    try:
        topic = sns_client.create_topic(Name=f"{PROJECT}-alerts")
    ...
    for name, arn in lambda_arns.items():
        ...
        try:
            cw_client.put_metric_alarm(
                AlarmName=f"{fn_name}-errors",
                AlarmDescription=f"Errors in {fn_name}",
                ...
                Statistic="Sum", TreatMissingData="notBreaching",
            )
            cw_client.put_metric_alarm(
                AlarmName=f"{fn_name}-duration",
                AlarmDescription=f"High latency in {fn_name}",
                MetricName="Duration", Namespace="AWS/Lambda",
                Dimensions=[{"Name": "FunctionName", "Value": fn_name}],
                Period=60, EvaluationPeriods=1,
                Threshold=5000, ComparisonOperator="GreaterThanThreshold",
                Statistic="Average", TreatMissingData="notBreaching",
            )
            print(f"  ✅ Alarms set: {fn_name}")
        except Exception as e:
            print(f"  ⚠️  Alarm skipped {fn_name}: {e}")
